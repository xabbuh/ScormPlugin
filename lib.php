            ...
            if ($scoes = $DB->get_records('scorm_scoes', $conditions, "id ASC")) {
                ...
                foreach ($scoes as $pos => $sco) {
                    ...
                    if (($nextsco !== false) && ($sco->parent != $nextsco->parent) && (($level==0) || (($level>0) && ($nextsco->parent == $sco->identifier)))) {
                        ...
                        $report .= '<img src="'.icon_url('spacer', 'scorm').'" alt="" />';
                    }
                    ...
                    if ($sco->launch) {
                        ...
                        if ($usertrack=scorm_get_tracks($sco->id, $user->id)) {
                            ...
                            $report .= "<img src='".icon_url($usertrack->status, 'scorm')."' alt='$strstatus' title='$strstatus' />";
                        } else {
                            if ($sco->scormtype == 'sco') {
                                $report .= '<img src="'.icon_url('notattempted', 'scorm').'" alt="'.get_string('notattempted', 'scorm').'" title="'.get_string('notattempted', 'scorm').'" />';
                            } else {
                                $report .= '<img src="'.icon_url('asset', 'scorm').'" alt="'.get_string('asset', 'scorm').'" title="'.get_string('asset', 'scorm').'" />';
                            }
                        }
